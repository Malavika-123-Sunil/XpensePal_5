...
function removeExpenseItem(id) {
    ...
    function addManualPayment(participantName) {
        ...
        if (amount && !isNaN(amount) && Number(amount) > 0) {
            ...
            const recipientParticipant = participants.find(p => p.name === recipient);
    
            if (participant && recipientParticipant) {
    ...
    function markPaymentComplete(index) {
        ...
        const amountPaid = prompt(`Enter the amount paid by ${settlement.from} to ${settlement.to}:`);
        if (amountPaid && !isNaN(amountPaid) && Number(amountPaid) > 0) {
            ...
            if (payer) {
                ...
                // Update the balance
                payer.balance += Number(amountPaid);
                settlement.amount -= Number(amountPaid);
